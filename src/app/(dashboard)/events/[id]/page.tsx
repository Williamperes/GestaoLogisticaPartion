...
import { toggleChecklistItem, promoteToReadyToLoad } from "@/app/(dashboard)/events/actions";
...
export default async function EventDetailPage({ params }: EventDetailPageProps) {
  ...
  const equipmentLocked = !isReadyToLoad;
    ...
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start">
        ...
        >
          ...
          {/* Botão de liberar carga (só aparece quando checklist está 100% e status ainda é planning) */}
            ...
            <ul className="divide-y divide-border">
              {event.checklist.map((item) => (
                <li key={item.id} className="flex flex-wrap items-center gap-3 px-5 py-4">
                  ...
                  {item.done ? (
                    <span className="text-[10px] font-semibold text-emerald-600 sm:ml-auto">
                      CONCLUÍDO
                    </span>
                  ) : (
                    <span className="text-[10px] font-semibold text-amber-500 sm:ml-auto">
                      PENDENTE
                    </span>
                  )}
                </li>
          ...
          {equipmentLocked ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-xl border border-amber-500/30 bg-amber-500/5 py-16 text-center">
              ...
              <p className="text-sm font-semibold text-amber-700">
                Lista de equipamentos bloqueada
              </p>
              <p className="mt-1 max-w-xs text-xs text-muted-foreground">
                Complete todos os{" "}
                <strong>{event.checklist.length - checklistDone} {event.checklist.length - checklistDone === 1 ? "item" : "itens"} pendente{event.checklist.length - checklistDone === 1 ? "" : "s"}</strong>{" "}
            ...
            <>
                ...
                <table className="min-w-[640px] w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {event.equipment.map((eq) => (
                      <tr key={eq.id} className="transition-colors hover:bg-black/2">
              ...
              <div className="mt-3 flex justify-end">
                <button className="flex items-center gap-2 rounded-lg border border-border px-4 py-2 text-sm text-muted-foreground transition-all hover:bg-black/5 hover:text-foreground">
