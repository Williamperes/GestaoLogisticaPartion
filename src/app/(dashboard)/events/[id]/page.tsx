  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex flex-col md:flex-row md:items-start gap-4">
        ...
        {/* Gate widget */}
        <div className={`shrink-0 border rounded-xl p-4 w-56 ${gateProgress === 100 ? "border-emerald-500/30 bg-emerald-500/5" : "border-amber-500/30 bg-amber-500/5"}`}>
            ...
            <ul className="divide-y divide-border">
              {event.checklist.map((item) => (
                <li key={item.id} className="flex items-center gap-3 px-5 py-3.5">
                  ...
                  {item.done && <span className="ml-auto text-[10px] text-emerald-600 font-semibold">CONCLUÍDO</span>}
                </li>
              ))}
            </ul>
          </div>
